...
describe 'LabelFormBuilder' do
  before do
    ...
    parent_builder = ActionView::Helpers::FormBuilder.new(:label, @project, @template, {}, nil)
    @builder = StarIt::LabelFormBuilder.new(:project, @project, @template, {:parent_builder => parent_builder}, nil)
  end
  ...
  context 'required fields' do
    ...
    it 'should add class star to label tag if no class is present' do
      expect(@builder.label(:name)).to eq('<label class="star_it " for="project_name">Name</label>')
    end

    it 'should append star class to existing class' do
      expect(@builder.label(:name, 'title', class: 'title')).to eq('<label class="star_it title" for="project_name">title</label>')
    end
  end
  ...
  context 'not required fields' do
    it 'should add class star to label tag if no class is present' do
      ...
      expect(@builder.label(:name)).to eq('<label for="project_name">Name</label>')
    end
  end
end
